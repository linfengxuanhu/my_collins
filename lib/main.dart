...
class Word {
  String word, source, definition, example, note;
  int status;
  Word({
    required this.word,
    this.source = '',
...
class _AppState extends State<App> {
  ...
  Future<void> save() async {
    final p = await SharedPreferences.getInstance();
    await p.setString(
        'words', jsonEncode(words.map((x) => x.toJson()).toList()));
  }
    ...
    setState(() {
      if (i < 0) {
        ...
        words[i] = Word(
          ...
          definition:
              w.definition.isNotEmpty ? w.definition : old.definition,
...
class Home extends StatefulWidget {
  ...
  final void Function(Word, int) status;
  const Home({
    super.key,
    required this.words,
    required this.dict,
...
class _HomeState extends State<Home> {
  ...
  Widget build(BuildContext c) => Scaffold(
        ...
        floatingActionButton:
            FloatingActionButton.extended(onPressed: _add, icon: const Icon(Icons.add), label: const Text('添加词')),
  ...
  Widget _home(BuildContext c) => ListView(
        ...
        children: [
              ...
              child: Column(
                ...
                children: [
                  ...
                  Wrap(spacing: 8, children: List.generate(4, (i) {
                    final cnt = widget.words
                        .where((w) => w.status.clamp(0, 3) == i)
                        .length;
                    return Chip(
                        label: Text('${['待抄', '已抄', '已描', '已掌握'][i]} $cnt'));
                  }))
                ],
              ),
            ),
          ...
          const SizedBox(height: 12),
          const Text(
              '操作提示：查到一个词后，可以加入“我的词库”，并填写它是从哪个词里发现的。')
        ],
      );

  Widget _library(BuildContext c) => ListView(
        padding: const EdgeInsets.all(12),
        children: widget.words.map((w) => _tile(c, w)).toList(),
      );

  Widget _tree(BuildContext c) => ListView(
        padding: const EdgeInsets.all(12),
        children: [
          Card(
            child: Padding(
  ...
  List<Word> _roots() => widget.words
      .where((w) =>
          ...
          !widget.words.any((x) =>
              x.word.toLowerCase() == w.source.toLowerCase()))
      .toList();

  Widget _treeNode(BuildContext c, Word w, int level, Set<String> path) {
    final keyStr = w.word.toLowerCase();
    if (path.contains(keyStr)) return const SizedBox.shrink();
    final next = {...path, keyStr};
    final kids = widget.words
        .where((x) => x.source.toLowerCase() == keyStr)
        .toList();
    final entry = widget.dict
            .where((e) => e.word.toLowerCase() == keyStr)
            .firstOrNull ??
        Entry(w.word, w.definition);
    ...
    return Column(
      ...
      children: [
        Padding(
          padding: EdgeInsets.only(left: level * 18),
          child: Card(
            child: ListTile(
              dense: true,
              ...
              subtitle: Text(
                  '${w.source.isEmpty ? '种子词' : '来自：${w.source}'} · ${['待抄', '已抄', '已描', '已掌握'][w.status.clamp(0, 3)]}'),
  ...
  void _search(String x) {
    ...
    final exact = widget.dict
        .where((e) => e.word.toLowerCase() == query)
        .toList();
    ...
    Navigator.push(
        ...
        MaterialPageRoute(
            builder: (_) => Results(
                  query: query,
                  exact: exact.isEmpty ? null : exact.first,
                  starts: starts,
                  all: widget.words,
                  add: widget.add,
                  status: widget.status,
  ...
  void _add() => Navigator.push(
      context, MaterialPageRoute(builder: (_) => Add(add: widget.add)));
}
...
class Results extends StatelessWidget {
        ...
        body: ListView(
          ...
          children: [
            ...
            if (starts.isNotEmpty) ...[
              ...
              ...starts
                  .where((x) => exact == null || x.word != exact!.word)
                  .map((x) => _card(c, x, false))
            ],
            if (exact == null && starts.isEmpty)
              const Padding(
                  padding: EdgeInsets.all(24),
  ...
  Widget _card(BuildContext c, Entry e, bool big) => Card(
        child: ListTile(
          ...
          subtitle: Text(e.definition,
              maxLines: big ? 12 : 3, overflow: TextOverflow.ellipsis),
          onTap: () => Navigator.push(
              c,
              MaterialPageRoute(
                  builder: (_) => DictPage(
...
class DictPage extends StatelessWidget {
  ...
  Widget build(BuildContext c) {
    final mine = all
        .where((w) => w.word.toLowerCase() == entry.word.toLowerCase())
        .firstOrNull;
    return Scaffold(
          ...
          FilledButton.icon(
            onPressed: () {
              ...
              ScaffoldMessenger.of(c).showSnackBar(SnackBar(
                  content: Text(sourceWord.isEmpty
                      ? '已加入我的词库'
                      : '已加入我的词库 · 来源：$sourceWord')));
            },
            icon: Icon(mine == null ? Icons.bookmark_add : Icons.bookmark),
            label: Text(mine == null
                ? '一键加入我的词库'
                : (sourceWord.isEmpty ? '再次保存' : '一键加入 / 更新来源')),
          ),
          if (mine != null) ...[
            ...
            Wrap(spacing: 8, children: List.generate(4, (i) {
              return ChoiceChip(
                label: Text(['待抄', '已抄', '已描', '已掌握'][i]),
                selected: mine.status.clamp(0, 3) == i,
                onSelected: (_) => status(mine, i),
              );
            }))
          ]
        ],
...
class _ClickableDefinition extends StatefulWidget {
  ...
  @override
  State<_ClickableDefinition> createState() => __ClickableDefinitionState();
}

class __ClickableDefinitionState extends State<_ClickableDefinition> {
  ...
  Widget build(BuildContext c) {
    ...
    final matches = re.allMatches(widget.text).toList();
    if (matches.isEmpty) {
    ...
    for (final m in matches) {
      if (m.start > pos) {
        spans.add(TextSpan(text: widget.text.substring(pos, m.start)));
      }
      final token = m.group(0)!;
      final clean = token.toLowerCase();
      final found = widget.dict
          .where((e) => e.word.toLowerCase() == clean)
          .firstOrNull;
      if (found == null || clean == widget.currentWord.toLowerCase()) {
        spans.add(TextSpan(
            text: token,
    ...
    if (pos < widget.text.length) {
      spans.add(TextSpan(text: widget.text.substring(pos)));
    }
    return SelectableText.rich(
      TextSpan(
        style: DefaultTextStyle.of(c).style.copyWith(
              fontSize: 16,
              height: 1.45,
            ),
        children: spans,
...
class Detail extends StatelessWidget {
  ...
  Widget build(BuildContext c) {
    final kids = all
        .where((x) => x.source.toLowerCase() == w.word.toLowerCase())
        .toList();
    return Scaffold(
      ...
      body: ListView(
        ...
        children: [
          ...
          Wrap(spacing: 8, children: List.generate(4, (i) {
            return ChoiceChip(
              label: Text(['待抄', '已抄', '已描', '已掌握'][i]),
              selected: w.status.clamp(0, 3) == i,
              onSelected: (_) => status(w, i),
            );
          })),
          const SizedBox(height: 18),
          ...
          ...kids.map((x) => ListTile(
                title: Text(x.word),
                subtitle: Text(
                    ['待抄', '已抄', '已描', '已掌握'][x.status.clamp(0, 3)]),
                onTap: () => Navigator.push(
                    c,
                    MaterialPageRoute(
                        builder: (_) => Detail(
...
class CopyWorksheetPage extends StatelessWidget {
  ...
  const CopyWorksheetPage(
      {super.key, required this.words, this.takeCount = 20});
  @override
  Widget build(BuildContext context) {
    final selected = words.take(takeCount).toList();
    return Scaffold(
...
class CobuildStudyCardPage extends StatefulWidget {
  final Word word;
  final void Function(Word, int) onStatusChange;
  const CobuildStudyCardPage(
      {super.key, required this.word, required this.onStatusChange});
  @override
...
class _CobuildStudyCardPageState extends State<CobuildStudyCardPage> {
  ...
  void initState() {
    super.initState();
    _currentStatus = widget.word.status.clamp(0, 3);
  }
  ...
  Widget build(BuildContext context) {
    ...
    final current = meanings[meaning.clamp(0, meanings.length - 1)];
    return Scaffold(
      ...
      body: ListView(
        ...
        children: [
          ...
          const SizedBox(height: 12),
          Wrap(spacing: 6, children: List.generate(4, (i) {
            return ChoiceChip(
              label: Text(['待抄', '已抄', '已描', '已掌握'][i]),
              selected: _currentStatus == i,
              onSelected: (_) {
                setState(() {
                  _currentStatus = i;
                });
                widget.onStatusChange(widget.word, i);
              },
            );
          })),
          const SizedBox(height: 18),
